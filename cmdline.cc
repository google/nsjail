...
static bool setupArgv(nsj_t* nsj, int argc, char** argv, int optind) {
	...
	if (optind < argc) {
		...
		nsj->njc.mutable_exec_bin()->clear_path();
		nsj->njc.mutable_exec_bin()->clear_arg();
		nsj->njc.mutable_exec_bin()->clear_arg0();
	}
...
std::unique_ptr<nsj_t> parseArgs(int argc, char* argv[]) {
	...
	for (;;) {
		...
		case 'E':
			...
			break;
		case 0x70A:
			nsj->njc.mutable_user_net()->set_backend(
			    nsjail::NsJailConfig_UserNet_Backend_NSTUN);
			break;
		case 'u': {
