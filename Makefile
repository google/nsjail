...
.PHONY: test test_mount_destination_confinement test_mount_attach_identity test_mount_precapture_identity test_legacy_mount_setattr_enosys
test: $(BIN)
	# --- Basic sanity tests ---
...
test_mount_destination_confinement: $(BIN)
	@cases='attack_mid_abs attack_mid_rel final_file final_dir safe_final_file_bind safe_final_dir_bind safe_final_dir_tmpfs bind_file bind_dir tmpfs dynamic_content symlink_dst ro_src positive_abs positive_rel loop nondir missing race root_dot parentdot nul'; \
	for clone_profile in default disable_clone_newuser; do \
		for backend in old new; do \
			for case_name in $$cases; do \
				for run_idx in 1 2; do \
					cmd="bash ./tests/mount_destination_confinement.sh ./nsjail $$backend $$case_name patched $$clone_profile"; \
					echo "Testing: $$cmd (run $$run_idx, expecting exit code 0)"; \
					($$cmd); ret=$$?; \
					if [ "$$ret" -ne 0 ]; then \
						echo "❌ FAIL: '$$cmd' returned $$ret, expected 0"; \
						exit 1; \
					else \
						echo "✅ PASS: '$$cmd' returned $$ret"; \
					fi; \
				done; \
			done; \
		done; \
...
test_mount_attach_identity:
	+$(MAKE) clean
	+$(MAKE) nsjail USER_DEFINES="$(USER_DEFINES) -DNSJAIL_TEST_HOOKS"
	@cases='bind_file bind_dir tmpfs'; \
	for clone_profile in default disable_clone_newuser; do \
		for backend in old new; do \
			for case_name in $$cases; do \
				for mode in restore persistent; do \
					for run_idx in 1 2; do \
						cmd="bash ./tests/mount_attach_identity.sh ./nsjail $$backend $$case_name patched $$clone_profile $$mode"; \
						echo "Testing: $$cmd (run $$run_idx, expecting exit code 0)"; \
						($$cmd); ret=$$?; \
						if [ "$$ret" -ne 0 ]; then \
							echo "❌ FAIL: '$$cmd' returned $$ret, expected 0"; \
...
test_mount_precapture_identity:
	+$(MAKE) clean
	+$(MAKE) nsjail USER_DEFINES="$(USER_DEFINES) -DNSJAIL_TEST_HOOKS"
	@cases='bind_file bind_dir tmpfs'; \
	for clone_profile in default disable_clone_newuser; do \
		for backend in old new; do \
			for case_name in $$cases; do \
				for exchange_point in leaf prefix; do \
					for mode in restore persistent; do \
						for run_idx in 1 2; do \
							cmd="bash ./tests/mount_precapture_identity.sh ./nsjail $$backend $$case_name fixed $$clone_profile $$exchange_point $$mode"; \
							echo "Testing: $$cmd (run $$run_idx, expecting exit code 0)"; \
							($$cmd); ret=$$?; \
							if [ "$$ret" -ne 0 ]; then \
								echo "❌ FAIL: '$$cmd' returned $$ret, expected 0"; \
...
test_legacy_mount_setattr_enosys: $(BIN)
	$(call run_test, strace -qq -f -o /tmp/nsjail_test_enosys_bind.strace -e fault=mount_setattr:error=ENOSYS ./nsjail $(OLD_EF) -q -Mo --chroot / -B /tmp --user 99999 --group 99999 -- /bin/bash -c 'touch /tmp/nsjail_test_enosys_bind && rm -f /tmp/nsjail_test_enosys_bind', 0)
	$(call run_test, strace -qq -f -o /tmp/nsjail_test_enosys_ro.strace -e fault=mount_setattr:error=ENOSYS ./nsjail $(OLD_EF) -q -Mo --chroot / -R /tmp --user 99999 --group 99999 -- /bin/bash -c 'touch /tmp/nsjail_test_enosys_ro || exit 77', 77)
	$(call run_test, strace -qq -f -o /tmp/nsjail_test_enosys_tmpfs_rw.strace -e fault=mount_setattr:error=ENOSYS ./nsjail $(OLD_EF) -q -Mo --chroot / -m none:/tmp:tmpfs:rw --user 99999 --group 99999 -- /bin/bash -c 'touch /tmp/nsjail_test_enosys_tmpfs && rm -f /tmp/nsjail_test_enosys_tmpfs', 0)
	$(call run_test, strace -qq -f -o /tmp/nsjail_test_enosys_tmpfs_ro.strace -e fault=mount_setattr:error=ENOSYS ./nsjail $(OLD_EF) -q -Mo --chroot / -m none:/tmp:tmpfs:ro --user 99999 --group 99999 -- /bin/bash -c 'touch /tmp/nsjail_test_enosys_tmpfs_ro || exit 77', 77)
